import { getSupabaseAdmin, STORAGE_BUCKET } from '@/lib/supabase-admin';
import { readLocalBranding } from '@/lib/local-branding-store';

export const runtime='nodejs';
export const dynamic='force-dynamic';
const hasSupabase=()=>Boolean(process.env.NEXT_PUBLIC_SUPABASE_URL&&process.env.SUPABASE_SERVICE_ROLE_KEY);

export async function GET(request:Request){
  try{
    if(!hasSupabase()&&['localhost','127.0.0.1','::1'].includes(new URL(request.url).hostname)){
      const result=await readLocalBranding('logo');
      return new Response(result.data,{headers:{'Content-Type':result.type||'image/png','Cache-Control':'no-store'}});
    }
    const result=await getSupabaseAdmin().storage.from(STORAGE_BUCKET).download('branding/logo');
    if(result.error)return new Response(null,{status:404});
    return new Response(result.data,{headers:{'Content-Type':result.data.type||'image/png','Cache-Control':'no-store'}});
  }catch{return new Response(null,{status:404})}
}

